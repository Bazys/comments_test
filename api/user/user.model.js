...
UserSchema.methods = {
  commentsList(callback) {
    return Comment.find({ creator_id: this.id }, callback);
  },
  /**
  ...
  authenticate(password, callback) {
    if (!callback) {
      console.log('!!!!!', this.password, this.encryptPassword(password), this.salt);
  ...
  encryptPassword(password, callback) {
    if (!password || !this.salt) {
      console.log('1111', this.password, this.salt);
      if (!callback) {
        return null;
      } else {
